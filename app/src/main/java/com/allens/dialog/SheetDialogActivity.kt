...
//                .setLineColor(Color.BLACK)//中间分割线的颜色
                .addSheetItem("照片", object : OnSheetItemClickListener() {
                    override fun onSheetItemClick() {
                        Toast.makeText(this@SheetDialogActivity, "照片", Toast.LENGTH_SHORT).show()
                    }
                })
                .addSheetItem("拍照", object : OnSheetItemClickListener() {
                    override fun onSheetItemClick() {
                        Toast.makeText(this@SheetDialogActivity, "拍照", Toast.LENGTH_SHORT).show()
                    }
                })
            for (i in 1..itemSize) {
                dialog.addSheetItem("item  $i", object : OnSheetItemClickListener() {
                    override fun onSheetItemClick() {
                        Toast.makeText(this@SheetDialogActivity, "item  $i", Toast.LENGTH_SHORT).show()
                    }
                })
            }
            dialog
